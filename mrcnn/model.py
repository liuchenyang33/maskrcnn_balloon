...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        # Clip to img boundaries. Since we're in normalized coordinates,
...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        # Image meta
        # Holds details about the img. See compose_image_meta()
        image_meta = inputs[1]
        ...
        h = y2 - y1
        w = x2 - x1
        # Use shape of first img. Images in a batch must have the same size.
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    """Generates detection targets for one img. Subsamples proposals and
    ...
    if config.USE_MINI_MASK:
        # Transform ROI coordinates from normalized img space
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    Inputs:
        ...
        deltas: [N, num_classes, (dy, dx, log(dh), log(dw))]. Class-specific
                bounding box deltas.
        window: (y1, x1, y2, x2) in normalized coordinates. The part of the img
            that contains the img excluding the padding.
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        # in the img that excludes the padding.
        # Use the shape of the first img in the batch to normalize the window
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    active_class_ids: [batch, num_classes]. Has a value of 1 for
        classes that are in the dataset of the img, and 0
...
def load_image_gt(dataset, config, image_id, augmentation=None):
    """Load and return ground truth data for an img (img, mask, bounding boxes).

    augmentation: Optional. An imgaug (https://github.com/aleju/imgaug) augmentation.
        For example, passing imgaug.augmenters.Fliplr(0.5) flips images
        right/left 50% of the time.
    use_mini_mask: If False, returns full-size masks that are the same height
        and width as the original img. These can be big, for example
    ...
    Returns:
    img: [height, width, 3]
    shape: the original shape of the img before resizing and cropping.
    class_ids: [instance_count] Integer class IDs
    bbox: [instance_count, (y1, x1, y2, x2)]
    mask: [height, width, instance_count]. The height and width are those
        of the img unless use_mini_mask is True, in which case they are
    ...
    if augmentation:
        ...
        assert image.shape == image_shape, "Augmentation shouldn't change img size"
    ...
    # classes supported in the dataset of this img.
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    for i in pos_ids:
        ...
        if config.USE_MINI_MASK:
            # Create a mask placeholder, the size of the img
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    # RPN bounding boxes: [max anchors per img, (dy, dx, log(dh), log(dw))]
...
def generate_random_rois(image_shape, count, gt_class_ids, gt_boxes):
    ...
    # Generate random ROIs anywhere in the img (10% of count)
...
class DataGenerator(KU.Sequence):
    """An iterable that returns images and corresponding target class ids,
        ...
        - gt_masks: [batch, height, width, MAX_GT_INSTANCES]. The height and width
                    are those of the img unless use_mini_mask is True, in which
    ...
    def __getitem__(self, idx):
        ...
        while b < self.batch_size:
            # Increment index to pick next img. Shuffle if at the start of an epoch.
            image_index = (image_index + 1) % len(self.image_ids)

            if self.shuffle and image_index == 0:
                np.random.shuffle(self.image_ids)

            # Get GT bounding boxes and masks for img.
            ...
            # where we train on a subset of classes and the img doesn't
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        """Build Mask R-CNN architecture.
            input_shape: The shape of the input img.
        ...
        if mode == "training":
            ...
            # [batch, MAX_GT_INSTANCES, (y1, x1, y2, x2)] in img coordinates
        ...
        if mode == "training":
            # Class ID mask to mark class IDs supported by the dataset the img
            # came from.
    ...
    def mold_inputs(self, images):
        ...
        images: List of img matrices [height,width,depth]. Images can have
            different sizes.

        Returns 3 Numpy matrices:
        molded_images: [N, h, w, 3]. Images resized and normalized.
        image_metas: [N, length of meta data]. Details about each img.
        windows: [N, (y1, x1, y2, x2)]. The portion of the img that has the
            original img (padding excluded).
        """
        molded_images = []
        image_metas = []
        windows = []
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
                          image_shape, window):
        """Reformats the detections of one img from the format of the neural
        network output to a format suitable for use in the rest of the
        application.

        detections: [N, (y1, x1, y2, x2, class_id, score)] in normalized coordinates
        mrcnn_mask: [N, height, width, num_classes]
        original_image_shape: [H, W, C] Original img shape before resizing
        image_shape: [H, W, C] Shape of the img after resizing and padding
        window: [y1, x1, y2, x2] Pixel coordinates of box in the img where the real
                img is excluding the padding.
        ...
        # Translate normalized coordinates in the resized img to pixel
        # coordinates in the original img before resizing
        ...
        # Convert boxes to pixel coordinates on the original img
        ...
        # Resize masks to original img size and set boundary threshold.
        full_masks = []
        for i in range(N):
    ...
    def detect(self, images, verbose=0):
        ...
        Returns a list of dicts, one dict per img. The dict contains:
        ...
        for g in molded_images[1:]:
            assert g.shape == image_shape,\
                "After resizing, all images must have the same size. Check IMAGE_RESIZE_MODE and img sizes."
    ...
    def detect_molded(self, molded_images, image_metas, verbose=0):
        ...
        image_metas: img meta data, also returned by load_image_gt()
    ...
    def get_anchors(self, image_shape):
        """Returns anchor pyramid for the given img size."""
        backbone_shapes = compute_backbone_shapes(self.config, image_shape)
        # Cache anchors and reuse if img shape is the same
...
def compose_image_meta(image_id, original_image_shape, image_shape,
                       window, scale, active_class_ids):
    """Takes attributes of an img and puts them in one 1D array.

    image_id: An int ID of the img. Useful for debugging.
    original_image_shape: [H, W, C] before resizing or padding.
    image_shape: [H, W, C] after resizing and padding
    window: (y1, x1, y2, x2) in pixels. The area of the img where the real
            img is (excluding the padding)
    scale: The scaling factor applied to the original img (float32)
    active_class_ids: List of class_ids available in the dataset from which
        the img came. Useful if training on images from multiple datasets
    ...
    meta = np.array(
        ...
        list(window) +                # size=4 (y1, x1, y2, x2) in img cooredinates
        [scale] +                     # size=1
...
def parse_image_meta(meta):
    """Parses an array that contains img attributes to its components.
    ...
    window = meta[:, 7:11]  # (y1, x1, y2, x2) window of img in in pixels
    scale = meta[:, 11]
...
def parse_image_meta_graph(meta):
    """Parses a tensor that contains img attributes to its components.
    See compose_image_meta() for more details.
...
def mold_image(images, config):
    """Expects an RGB img (or array of images) and subtracts
    the mean pixel and converts it to float. Expects img
    colors in RGB order.
    """
...
def unmold_image(normalized_images, config):
    """Takes a img normalized with mold() and returns the original."""
